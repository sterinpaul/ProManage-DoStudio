...
export const SubTask = ({
    ...
    const updateName = async (event) => {
        ...
        if (subTaskName.trim().length) {
            ...
            if (subTask.task !== subTaskName) {
                ...
                if (response?.status) {
                    setSelectedProject(previous => previous.map(task => task._id === taskId ? { ...task, subTasks: task.subTasks.map(subTasks => subTask._id === subTasks._id ? { ...subTasks, task: subTaskName } : subTasks) } : task))
                    setEditToggle(false)
                } else {
    ...
    return (
        <tr className="even:bg-blue-gray-50 odd:bg-gray-100 hover:bg-white">
            ...
            {headers?.map(header => {
                if (header.key === "task") {
                    return <React.Fragment key={header._id}>
                        <td className={`${nameError && "outline-2 h-full outline-dashed outline-red-600"} ${classes} cursor-pointer w-60 p-0`}>
                    ...
                    return <SelectComponent key={header._id} currentValue={subTask.status} valueGroup={statusGroup} updateSubTaskOption={updateSubTaskOption} headerType={"status"} classes={classes} isAdmin={true} permission={true} addOptionModalToggle={addOptionModalToggle} />
                } else if (header.key === "dueDate") {
                    return <td key={header._id} className={`${classes} text-center w-36`}>
                    ...
                    return <td key={header._id} className={`${classes} text-center p-0.5`}>
                        ...
                        <div onClick={peopleModalhandler} className="relative">
                            {subTask?.peopleName ? <div className="relative group w-full h-full">
                                ...
                                : (
                                    <Avatar className="w-6 h-6" src="/avatar-icon.jpg" alt="ProfilePhoto" size="sm" />
                                )
                            }
                    ...
                    return <td key={header._id} className={`${notesError && "outline-2 outline-dashed outline-red-600"} ${classes} relative group cursor-pointer`}>
                    ...
                    return <DynamicSubTask key={header._id} classes={classes} dynamicValue={subTask[header.key]} field={header.key} taskId={taskId} subTaskId={subTask._id} updateDynamicField={updateDynamicField} />
                }
            })}
